...
[![Status: GA v1.0.0](https://img.shields.io/badge/status-GA%20v1.0.0-green)](RELEASE-NOTES.md)
...
**This is the v1.0.0 GA release** — the 1.0 bar is an integrity bar, not a feature bar; pin a tag. See [RELEASE-NOTES.md](RELEASE-NOTES.md) for exactly what is claimed, what is not, and the evidence for both.
...
Full detail — the ZIP-first install, the directory-not-empty case, and a file map — is in [QUICKSTART.md](sbak/QUICKSTART.md). The short version, **from the recommended `sbak-v1.0.0-starter.zip`** (verify the `.sha256` first):

```
unzip sbak-v1.0.0-starter.zip -d my-project && cd my-project
git init
...
|---|---|---|---|---|---|---|
| **Lite** | Under a week, low audit needs | ~44 | per pull request | brief paragraph | changelog only | ~10 min |
| **Full** (default) | a second reader, an audit need, or a multi-week horizon | ~108 | per stage | process + product | append-only advisory; CI-enforced when a risk trigger is declared | ~30 min |
| **Full** | Long-lived or regulated | ~108 | per stage | process + product + forward-readiness | append-only, enforced in CI | ~60–90 min |

Details and every toggle: [FRAMEWORK-CONFIG.md](sbak/FRAMEWORK-CONFIG.md).

**The overhead is real and it has been measured.** In a controlled two-arm trial on a small **solo** markdown-to-HTML CLI, the framework arm cost **≈13.6× the dollars** and **≈10.9× the API time** of a lightweight control loop, and the trial's own ratified adjudication FAILED two of six thresholds — adaptability and economic justification — **for that project class** (it passed the other four, including one evidence-bound prevented defect). If your project looks like that one, use a plain session; see [WHY-THIS-KIT.md](sbak/WHY-THIS-KIT.md) and [limitations.md](sbak/docs/limitations.md). The full trial record lives in `docs/trial/` **in this kit's source repository** — it is not part of this distributed release.
...
**Recommended:** download `sbak-v1.0.0-starter.zip` and its `.sha256` from the releases page and verify before you unpack — the `.sha256` checksum, and stronger, `gh attestation verify sbak-v1.0.0-starter.zip -R kknipe2k/Software-Build-Assurance-Kit` (SLSA build-provenance attestation). Node.js is the only runtime requirement for the validators and hooks; the kit itself is stack-agnostic.
