...
//   Overrides (for smoke mutations): --rows P  --claude-md P  --known-gaps P  --src-root D
//
...
OPT.claudeMd = OPT.claudeMd || path.join(OPT.srcRoot, 'CLAUDE.md');
...
function threeWayDiff() {
  ...
  try {
    ...
    // (2) row-set ⟷ CLAUDE.md tables (global set comparison).
    const tableFiles = new Set(parsePhase3Tables(fs.readFileSync(OPT.claudeMd, 'utf8')).map((r) => r.file));
    for (const rf of [...rowFiles].sort()) if (!tableFiles.has(rf)) push('table_missing', rf, 'in the row-set (the contract) but absent from the CLAUDE.md Phase-3 tables');
    for (const tf of [...tableFiles].sort()) if (!rowFiles.has(tf)) push('table_extra', tf, 'in the CLAUDE.md Phase-3 tables but absent from the row-set');
...
function main() {
  if (has('--dump-tables')) {
    const rows = parsePhase3Tables(fs.readFileSync(OPT.claudeMd, 'utf8'));
...
module.exports = {
  ...
  parsePhase3Tables, loadRows, applicable,
