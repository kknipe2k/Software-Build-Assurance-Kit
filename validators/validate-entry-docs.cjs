...
//   node validators/validate-entry-docs.cjs --write-manifest    (re)write repo-root framework-manifest.json
//
...
const ENTRY_DOCS = [
  ...
  // The governance surface. These four make counted, versioned claims to a
  // reader who has not cloned anything yet — RELEASE-NOTES states the gate count
  // and the SLSA level, SECURITY states what the release controls are. They were
  // unpoliced, so a drifted number there would have shipped as fact.
  'RELEASE-NOTES.md', 'CONTRIBUTING.md', 'SECURITY.md', 'CODE_OF_CONDUCT.md',
];
...
function entryDocSetConsistency(root, opts) {
  ...
  const violations = [];
  // (a) dangling: every set member resolves to a real file
  for (const rel of set) {
    if (!fs.existsSync(path.join(root, rel))) violations.push({ kind: 'dangling', file: rel });
  }
  // (b) narrowing: every present top-level *.html is in the set (evidence artifacts excepted)
  let htmls = [];
  try { htmls = fs.readdirSync(root).filter((n) => /\.html$/i.test(n)); } catch (_) { htmls = []; }
...
// Scan the whole entry-doc set (+ ledger files in staged-added mode). Returns { findings, manifest }.
...
function main() {
  ...
  const { findings } = scanTree(KIT_ROOT, { ledger: true });
  ...
  const setViolations = entryDocSetConsistency(KIT_ROOT);
