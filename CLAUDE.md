...
**Step 0.0 — Operating mode (leading question, asked first).** Before the five calibration choices, ask the leading question — *"What kind of work is this?"* — per the `sbak/templates/CALIBRATION-INTERVIEW.md` mode table. The answer sets `operating_mode` and routes the rest of Phase 0:
...
**Step 0.1 — Calibration interview (every project, all tiers).** Present `sbak/templates/CALIBRATION-INTERVIEW.md` verbatim — five choices, one question per turn, tables never paraphrased. Flag toggle smells; confirm in plain English; don't lock `project-config.md` until the spec lands. Detail: `sbak/bootstrap/PHASES.md` §Phase 0.
...
The pre-flight disclosure (rule #11, ask-and-WAIT), the category turn boundaries, and the four on-approval write steps: same section.
