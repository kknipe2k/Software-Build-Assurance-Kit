...
The project uses a **two-layer prompt structure** described in `persistence-architecture.md`:
...
- `BUILD-PLAYBOOK.md` (repo root) — methodology
- `STAGE-PROMPT-PROTOCOL.md` (repo root) — XML schema for X.5 stage prompts
- `PROCESS-VALIDATION.md` (repo root) — framework for evaluating whether the prompt-driven pattern works
- `persistence-architecture.md` (repo root) — layer model: how memory/instructions persist across sessions
