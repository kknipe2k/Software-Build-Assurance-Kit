...
> Pass 4 of the 8-dimension audit. Instantiates `templates/AUDIT-PASS-PROMPT-TEMPLATE.md` for the **data-flow** dimension; checklist seeded from `CODEBASE-AUDIT.md` (G3 data flow & state + S2 injection & input validation). Paste into a **fresh** session with `role: verifier` after Phase S. Fill `{{NN}}` and `{{H}}`. Validates against `STAGE-PROMPT-PROTOCOL.md` §8.7.
