...
> Pass 1 of the 8-dimension audit. Instantiates `templates/AUDIT-PASS-PROMPT-TEMPLATE.md` for the **IPC** dimension; checklist seeded from `CODEBASE-AUDIT.md` (S6 network surface + S2 injection over a boundary). Paste into a **fresh** session with `role: verifier` after Phase S. Fill `{{NN}}` (milestone) and `{{H}}` (time-box); the read-first paths assume the standard `docs/audit/` layout. Validates against `STAGE-PROMPT-PROTOCOL.md` §8.7.
