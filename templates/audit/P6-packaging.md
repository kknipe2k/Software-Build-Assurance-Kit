...
> Pass 6 of the 8-dimension audit. Instantiates `templates/AUDIT-PASS-PROMPT-TEMPLATE.md` for the **packaging** dimension; checklist seeded from `CODEBASE-AUDIT.md` (S5 dependencies & supply chain + S8 configuration, deployment & CI). Paste into a **fresh** session with `role: verifier` after Phase S. Fill `{{NN}}` and `{{H}}`. Validates against `STAGE-PROMPT-PROTOCOL.md` §8.7.
