...
> The build prompt for milestone M[NN]. Markdown wrapper for human readability + XML stage CLI prompts (per `STAGE-PROMPT-PROTOCOL.md`) for paste-into-session execution. Authored at the prior milestone's closeout (or at project bootstrap for M01).
>
...
**Protocol version:** v1.8 (per `STAGE-PROMPT-PROTOCOL.md` changelog).

> This banner is **load-bearing for two version-gated gates**: **G9** (test-honesty) and **G13** (the risk-matrix). Because it declares **v1.7+**, every work stage below must carry a `<test_honesty>` slot (a named mutation, or the explicit `n/a — no risk surface` sentinel); and because it declares **v1.8+**, a work stage that **touches a risk surface** (a `risk_triggers:` surface, `FRAMEWORK-CONFIG.md` §4.19) must carry a `<risk_declaration>` covering the 9-property matrix. `validators/validate-test-honesty.cjs` (G9) and `validators/validate-risk-matrix.cjs` (G13) block a non-compliant work stage. **Banner-less docs comply:** a banner-LESS doc is treated as *current* (must comply), not grandfathered — only an *explicit* pre-bump banner (`v1.7`, `v1.6`, …) grandfathers the doc out of the later gates (use only when re-issuing a genuinely older doc).
...
Per `BUILD-PLAYBOOK.md` §3.2, every stage follows the same loop:
...
<work_stage_prompt id="M[NN].A">
  ...
  <!-- v1.8 G13 — REQUIRED when this stage TOUCHES a risk surface (a
       risk_triggers: surface, FRAMEWORK-CONFIG.md §4.19: destructive data ops / archives /
...
Output: `retrospectives/M[NN].V-findings.md` using `templates/VERIFIER-FINDINGS-TEMPLATE.md`. Required tier-coverage caveat at top of file naming passes run, passes NOT run, bug classes NOT checked.
...
<verifier_stage_prompt id="M[NN].V">
  ...
  <read_first>
    <file>BUILD-PLAYBOOK.md §3.4 (verifier protocol)</file>
    <file>STAGE-PROMPT-PROTOCOL.md §8.5 (verifier-only tags)</file>
  ...
  <findings_format ref="templates/VERIFIER-FINDINGS-TEMPLATE.md">
  ...
  <retrospective_requirements ref="templates/VERIFIER-RETROSPECTIVE-TEMPLATE.md"/>
  <commit_protocol ref="BUILD-PLAYBOOK.md" section="4.7 Do-Not-Commit Rule"/>
...
> Optional and trigger-based — include this section only when `refactor_mode` is not `skip`. Stage R runs when `docs/tech-debt.md` has ≥ the threshold count OR the milestone interval has elapsed, whichever first (`FRAMEWORK-CONFIG.md` §4.11, `BUILD-PLAYBOOK.md` §3.4.5). Parallel to Stage V but asks **"is the code maintainable?"** against the *cumulative* codebase, not "did this milestone do what was promised?"
...
Output: `retrospectives/M[NN].R-findings.md` using `templates/REFACTOR-FINDINGS-TEMPLATE.md`. Required tier-coverage caveat at top of file naming passes run, passes NOT run, structural classes NOT checked.
...
<refactor_stage_prompt id="M[NN].R">
  ...
  <read_first>
    <file>BUILD-PLAYBOOK.md §3.4.5 (refactor health check)</file>
    <file>STAGE-PROMPT-PROTOCOL.md §8.6 (refactor-only tags)</file>
  ...
  <findings_format ref="templates/REFACTOR-FINDINGS-TEMPLATE.md">
  ...
  <retrospective_requirements ref="templates/REFACTOR-RETROSPECTIVE-TEMPLATE.md"/>
  <commit_protocol ref="BUILD-PLAYBOOK.md" section="4.7 Do-Not-Commit Rule"/>
...
- New entry in `docs/gap-analysis.md` (per `templates/gap-analysis.md` entry shape — six required sections)
...
<closeout_stage_prompt id="M[NN].E">
  ...
  <off_track_check gate="G8" ref="BUILD-PLAYBOOK.md" section="3.5">
  ...
  <scope_locks>
    <lock>Append-only is a hard rule (BUILD-PLAYBOOK.md §4.1, §4.6) — no editing prior entries, ever</lock>
