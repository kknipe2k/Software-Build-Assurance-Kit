...
- **This file** holds the **graduated** gotchas — ones that emerged from per-stage gotchas at closeout (per `<gotchas_graduation>` in `BUILD-PLAYBOOK.md` §3.4) and apply to multiple future stages.
- **Disposition values for per-stage gotchas at closeout** (per `STAGE-PROMPT-PROTOCOL.md` §8):
