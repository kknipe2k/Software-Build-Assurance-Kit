...
> Filled by Claude during the Refactor stage (Stage R — the refactor health check). Output artifact for `<findings_format>` per `STAGE-PROMPT-PROTOCOL.md` §8.6. Reviewed alongside the milestone's code diff. Distinct from `retrospectives/M[NN].V-findings.md` (Stage V — contract fidelity): Stage R asks **"is the code maintainable?"** against the *cumulative* codebase, not "did this milestone do what was promised?"
