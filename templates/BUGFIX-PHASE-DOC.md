...
> The build prompt for a **bug fix**, not a greenfield milestone. Markdown wrapper for human readability + XML stage CLI prompts (per `STAGE-PROMPT-PROTOCOL.md`) for paste-into-session execution.
>
...
**Protocol version:** v1.5 (per `STAGE-PROMPT-PROTOCOL.md`).
...
- Run a **fan-out grep** across the names / fields / types involved in the bug (`STAGE-PROMPT-PROTOCOL.md` §7 `<fan_out_grep>`). Surface every file that touches the affected surface.
...
Output: summarize to the owner (or `retrospectives/{{BUG_ID}}.C-findings.md` if requested), using `templates/VERIFIER-FINDINGS-TEMPLATE.md`. Required tier-coverage caveat naming which passes ran and which bug classes are NOT checked.
...
<verifier_stage_prompt id="M[NN].C">
  ...
  <read_first>
    <file>BUILD-PLAYBOOK.md §3.4 (verifier protocol), §3.7 (bug_fix workflow)</file>
...
*This Phase doc is the single source of truth for bug fix `{{BUG_ID}}`. There is no spec, scope, milestone plan, or closeout — three phases, one PR. Keep `docs/bugfix/{{BUG_ID}}.md` current; it is the bug's contract.*
