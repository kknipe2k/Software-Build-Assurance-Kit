...
## Framework principle (from BUILD-PLAYBOOK.md §3.3)
...
Per-stage CLI prompts reference gates by milestone: `<gates milestone="M[NN]"/>`. The validator (per `STAGE-PROMPT-PROTOCOL.md` §11) confirms the milestone tag matches an entry in this file.
...
| Dependency audit | `{{DEP_AUDIT_COMMAND}}` (e.g. `npm audit --audit-level=high` / `pip-audit` / `cargo audit`) | no high/critical | Standard+; Verifier Pass 6 mechanical floor |
| Secret scan | `{{SECRET_SCAN_COMMAND_OR_NA}}` (e.g. `gitleaks detect`) | no findings | Standard+; Verifier Pass 6 mechanical floor |
...
The class is read from the App-Map's `surface class:` stamp (or `--surface-class`); an unresolved class leaves the gate **un-armed** with a visible note, so a stamp-less legacy map never false-blocks. The arming is **always visible** ("cluster-gate armed because surface class = X") — never silent. The Evidence reference is the **same** one Stage C structures into its reconcilable evidence block; the cell binds B's gate to C's block so the mechanical cell can't be satisfied by free-text invented independent of a run. The trigger list (which classes arm) is in `FRAMEWORK-CONFIG.md` §4.18.
