...
//   node scripts/build-receipts.cjs render [range] [--out dir] [--name base]
...
function normalize(input) {
  ...
  // gates — the demonstrated controls + the verifier-finding count.
  const controls = [];
  ...
  const gates = { controls: controls, verifier_findings: verifierFindings };
...
function buildModel(cwd, range) {
  const arc = collect.collectArc({ cwd: cwd, range: range || 'HEAD' });
  const receiptsDir = path.join(cwd, '.claude', 'receipts');
  let events = [];
  const extraCoverage = [];
  if (fs.existsSync(receiptsDir)) {
    const m = receipts.mergeLedgers(receiptsDir);
    events = m.events;
...
function runRender(args) {
  ...
  const positional = args.filter((a, i) => !a.startsWith('--') && args[i - 1] !== '--out' && args[i - 1] !== '--name');
  const range = positional[0] || 'HEAD';
  const model = buildModel(cwd, range);
  const files = {};
