...
// garbage role fails closed — it does NOT fall through). This alias-window ends at
// v0.2.0, the first release after v0.1.0, when the legacy `.claude/active-mode` write
// (and the readers' fallback) is retired. This header comment is the writer's record
// of the window; the state-file contract note in persistence-architecture.md is the
...
writeMarkerAtomic('active-mode'); // alias-window: retired at v0.2.0
