...
## 0.1 The three tiers (canonical reference: `FRAMEWORK-CONFIG.md`)

- **Lite** — solo, simple/small project, ≤1 week, low audit needs. One-paragraph retrospective. No append-only ledger. Per-PR approval (not per-stage). Web-verified defaults (the framework leans on the web for non-experts). Bootstrap generates ~5 files. Recurring overhead per stage: ~10 minutes of human review.
- **Standard** — solo or small team, 1–4 weeks, intermediate complexity. Two-axis retrospective (process + product). Append-only ledger documented but not CI-enforced. Per-stage approval. Web-verified defaults. Bootstrap generates ~10 files. Recurring overhead per stage: ~30 minutes of human review.
- **Full** — multi-week to multi-month, high complexity OR high audit needs, expert user. Three-axis retrospective. Append-only ledger CI-enforced. Per-stage approval. Token-frugal research mode (you flag what to verify). Bootstrap generates ~14 files. Recurring overhead per stage: ~75 minutes of human review.

Pick by the highest-tier input across complexity, time horizon, and audit needs. A 3-day project with regulatory audit needs is **Full**, not Lite. Don't know? Start **Standard**.
...
- Standard tier + you want token-frugal research → flip `research_mode` to `token_frugal`
...
> **Tier note.** The orchestrator/build split is a **Standard and Full** construct — separate session types, separate operating manuals, the human conduiting between them. See `ORCHESTRATOR.md` §1 for how the two roles are physically hosted (two CLIs + git worktrees on one machine is the recommended topology). At **Lite** tier the two roles collapse into a single session: small project, markdown task lists, per-PR approval — there is no separate orchestration session and `ORCHESTRATOR.md` is not generated.
...
> **Tier note.** Required at Standard and Full (`verifier_mode: pass_1_2_4` and `pass_1_2_3_4` respectively). Optional at Lite (`pass_1_only`). The bug class Verifier catches (tests pass; contract doesn't) is tier-independent; what scales by tier is the *intensity* of verification, not whether it happens.
...
> **Tier note.** Trigger-based at Standard (`refactor_mode: trigger_n5`) and Full (`trigger_n3`); skipped at Lite (`refactor_mode: skip`). Schema: `STAGE-PROMPT-PROTOCOL.md` §8.6.
...
Tier-conditional: Lite skips; **Standard** runs Duplication + Drift (Complexity only if a linter is named); **Full** runs all three + project-specific passes from `docs/gates.md`.
...
- **Release-state climb + honest rework (Standard+ tiers):** the closeout records the milestone's place on the **release-state ladder** in the append-only `docs/release-state.md` (a `milestone-complete` entry, citing the gate it passed), and writes the milestone's **rework count across the four fixed types** — implementation corrections / verifier iterations / IRL reversals / post-merge discoveries (DORA's 5th metric, project-internal) — which `validators/validate-transition.cjs` (G15) **reconciles against the fix-commit evidence**: a total that under-reports the fix commits, or a "0 rework" claim while fix commits exist, fails the closeout. See §"Release-state ladder" below.
...
`per_milestone` (Full default) writes a full gap-analysis entry at every milestone close. `per_release` (**Standard default**) writes only the milestone summary + `CHANGELOG.md` at intermediate closeouts and appends the cumulative gap-analysis entry once, at the **v1-release** closeout — where the carry-forward summarization actually earns its cost. Standard intermediate closeouts skip the six-section ledger entry; the summary + CHANGELOG carry the forensic role at lower fidelity until release.
...
### Release-state ladder (Standard+ tiers)
...
1. **Phase A — Reproducer + impact analysis** (`role: work`). Write a regression test that reproduces the bug and **fails for the right reason** (G_BUGFIX_A1) — the assertion about the buggy behavior, not a setup error or a skipped test. Run a fan-out grep across the names / fields / types involved; surface every file touching the affected surface. Produce the **impact-analysis list**: the existing tests to re-run after the fix. This list is the TDAD differentiator — the contextual "which tests to re-run" that helps a smaller model far more than a bare "do TDD" instruction.
2. **Phase B — Minimal fix** (`role: work`). The smallest change that turns the regression test red→green. **Scope lock (G_BUGFIX_B1):** no unrelated refactors, no "while we're here" cleanups; out-of-scope improvements append to `docs/tech-debt.md` (or `CHANGELOG.md` at Lite), never folded in. Re-run the Phase-A impact list — all green.
...
The *cadence* of approval (per-step / per-stage / per-milestone / per-PR) follows the `approval_cadence` toggle in `project-config.md`. Tier defaults: Lite = `per_pr`, Standard = `per_stage`, Full = `per_stage`. Whatever cadence you've configured, the agent surfaces and waits at that boundary; no commits cross the boundary unapproved.
...
> **Tier note.** This check protects multi-machine orchestration setups (orchestration session reads origin while build machine has uncommitted work). It applies in Standard and Full tiers. Lite projects typically run on one machine with one operator; this check can be skipped (set `cross_machine_check: skip` in `project-config.md` if your Lite project ever needs it disabled explicitly).
