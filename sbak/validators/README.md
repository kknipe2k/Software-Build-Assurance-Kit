...
| `validate-irl-plan.cjs` | the IRL/HITL presence floor (M29.A): a Full-tier `spec/project-spec.md` must carry the three-part IRL/HITL plan — drive moments per boundary, what the human verifies by hand, where each answer gets typed — **each part individually** (heading + at least one table row; a bare section heading satisfies nothing). Detection is shared with the closeout consumption gate via `lib/irl-plan.cjs`. `--staged` mode checks only when a spec file is staged (the M12 retro-fail guard: history is never retro-failed). Lite / spec-less / no project-config → visible n/a, never a silent pass | Full tier (pre-commit; Lite warn) |
| `validate-claude-integrity.cjs` | the CLAUDE.md self-integrity gate: a staged root `CLAUDE.md` under half its committed byte size BLOCKS (the truncated-rules class — the field incident deleted every hard rule and merged unnoticed; override with `SBAK_ALLOW_CLAUDE_SHRINK="<reason>"`, reason logged, never silent), and `.claude/settings.json` must keep all four session hooks registered (the dormant-layer class; absent settings = visible note, honor-system hosts). Honest scope: a byte-count tripwire + registration parse, not a semantic diff — the human review is the wall | all tiers (pre-commit) |
| `validate-closeout-packet.cjs` | the closeout floor (fades 2+4, rows 48/49): `--packet` enforces the three hard rules (LEADS with deltas-from-plan + open V/R findings; `reviewed-against: receipts`; the CHANGELOG `derived-from: git log` + the one `why-it-matters:` line); `--gap` enforces the gap-analysis content contract (cites receipts/retro/V-R, counts seeded via a reconcile block); `--ladder <MNN>` REDs a closeout whose release-state ledger has no row for the closing milestone. Honest locus: presence over a stated shape — it cannot grade the synthesis; the owner's packet review is that wall | Full (closeout) |
| `validate-gate-manifest.cjs` | the gate-command manifest (the general form of the recorded local-vs-CI gate-drift class): derives the local gate set from `.githooks/*` and the CI set from `.github/workflows/*.yml` at check time (hand-listed rows rot) and BLOCKS a divergence — a validator invoked locally with no CI path to it, or vice versa; a workflow running an aggregate suite (verify-local / smoke) covers the estate at the aggregator level (stated, not hidden). No workflows at all = visible note (local-only projects) | all tiers |
