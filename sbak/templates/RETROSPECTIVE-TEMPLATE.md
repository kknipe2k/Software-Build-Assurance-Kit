...
> Per-stage retrospective. Claude maintains this during the session and surfaces it at stage end alongside the commit-ready surface. The user reviews; does not fill in fields. Per `sbak/BUILD-PLAYBOOK.md` §3.5.
...
These were satisfied before code was written. If `<pre_flight_check>` was specified in the CLI prompt (per `sbak/STAGE-PROMPT-PROTOCOL.md` v1.3+), each check there should map to a checkbox here.
...
Numbered list of gotchas that surfaced during this stage. These get evaluated for graduation at milestone closeout (per `<gotchas_graduation>` in `sbak/BUILD-PLAYBOOK.md` §3.4).
...
### Threshold gates (per `sbak/BUILD-PLAYBOOK.md` §3.5)
...
### Off-track check (priority drift — per `sbak/PROCESS-VALIDATION.md` G8)
...
If the stage surfaced a decision that should propagate beyond the next stage — into `CLAUDE.md`, `docs/style.md`, `docs/gotchas.md`, `sbak/BUILD-PLAYBOOK.md`, or a new ADR — list here. The user routes these at outcome review.
...
*This retrospective becomes input to the next stage's CLI prompt (read via `<read_prior_stages>` per `sbak/STAGE-PROMPT-PROTOCOL.md` §7) and to the milestone-level summary at closeout (per `prompts/SUMMARY-TEMPLATE.md`).*
