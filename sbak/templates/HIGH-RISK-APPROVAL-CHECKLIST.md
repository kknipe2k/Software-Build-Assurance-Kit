...
>
> **What it is NOT.** This is **human oversight, not technical enforcement.** The M08 permission fence (`.claude/settings.json`) still applies underneath — `git commit`/`git push` stay `ask`, the deny floor still walls off secret reads and irreversible Bash — but the fence's four honest caveats hold (see `CLAUDE.md` §6.5): the deny floor has bypasses, `.claudeignore` is broken, a web-remote session ignores repo-set `auto`, and `allow` is inert until the workspace trust dialog is accepted. The checklist closes the *judgment* gap (the approver thinking before clicking), not a technical one. Don't read a filled checklist as a guarantee the op is safe — read it as evidence the approver looked.
...
- Has the rollback path itself been tested — not assumed? (The destructive-op hard rule, `sbak/BUILD-PLAYBOOK.md`: rollback **and** confinement are two independent tests; a passing rollback test is not evidence of confinement.)
