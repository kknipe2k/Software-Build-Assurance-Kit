...
> Pass 3 of the 8-dimension audit. Instantiates `sbak/templates/AUDIT-PASS-PROMPT-TEMPLATE.md` for the **error-handling** dimension; checklist seeded from `CODEBASE-AUDIT.md` (G2 error handling & resource safety). Paste into a **fresh** session with `role: verifier` after Phase S. Fill `{{NN}}` and `{{H}}`. Validates against `sbak/STAGE-PROMPT-PROTOCOL.md` §8.7.
