...
2. Run the verification passes named there at the project's `verifier_mode` (default Full `pass_2_4` = hooks + behavior + security + code_quality — **no inventory pass** (§X.2 is advisory at the default); add `design` when `deliverable_type: web`; `pass_1_2_3_4` (explicit choice) adds inventory + multi-call; Lite `pass_1_only`). For `deliverable_type: web`, Pass 4 requires a literal observed-running step and Pass 5 the design-conformance check against `docs/design.md`.
