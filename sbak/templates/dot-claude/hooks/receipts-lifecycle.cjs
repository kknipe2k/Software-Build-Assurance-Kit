...
function main() {
  ...
  // Release-origin classification (UAT #15, M22.A; anchored M22.B): an in-session
  // invocation of a red-gate release command is marked with a BOUNDED value from
  // the contract's RELEASES enum. The command must START with the release
  // invocation (modulo leading whitespace) — the same prefix semantics the
  // permission fence applies — so a command that merely MENTIONS a script
  // (echo/grep) or embeds it in a compound (cd x && node …) never classifies;
  // the compound false-negative direction is the ratified trade (the fence still
  // asks on the compound). The command text is read here at observation and never
  // persisted (same shape as tool_name → tool_category, J4). Opening a stage
  // (stage-active.cjs WITHOUT --clear) is not a release. A red_approved /
  // stage_cleared script event with NO such marker = the human's outside shell.
  if (hookName === 'PreToolUse' && payload.tool_name === 'Bash'
      && payload.tool_input && typeof payload.tool_input.command === 'string') {
    const cmd = payload.tool_input.command;
    if (/^\s*node scripts[\/\\]approve-red\.cjs(\s|$)/.test(cmd)) raw.release = 'approve-red';
    else if (/^\s*node scripts[\/\\]stage-active\.cjs\s/.test(cmd) && /(^|\s)--clear(\s|$)/.test(cmd)) raw.release = 'stage-clear';
  }

  // The RED-RELEASE line-marker (ruling 5, M22.G): a UserPromptSubmit whose
  // prompt carries the EXACT release line — anchored as its own line; a
  // paraphrase never classifies — lands a SECOND bounded event alongside
  // turn_started: release_line_observed, line-present classification only.
  // The prompt text is read here at observation and never persisted (the
  // privacy allowlist has no prompt field). Report-side three-state reading:
  // a hook-observed release preceded by this marker in the same session =
  // in-session WITH the line (the ruling-5-sanctioned path); without it =
  // the self-release tell; a script event with no hook marker at all = the
  // human's outside shell.
  const lineMarker = hookName === 'UserPromptSubmit'
    && typeof payload.prompt === 'string'
    && /^\s*RED-RELEASE: approved — builder, run node scripts\/approve-red\.cjs\s*$/m.test(payload.prompt);
