...
The arming is **visible** ("cluster-gate armed because surface class = X"), keyed off the derived surface class in the `surface class:` stamp above (or `--surface-class`); the trigger list lives in `sbak/FRAMEWORK-CONFIG.md` §4.18 + `docs/gates.md`. **Honest limitation:** the validator can only assert the reference is *present* — it cannot run the app, so the run-reality is the verifier's assembled_execution pass and Stage C's reconciliation, not this static cell.
