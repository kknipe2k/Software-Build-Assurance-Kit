...
> Filled by Claude during the Verifier stage. Output artifact for `<findings_format>` per `sbak/STAGE-PROMPT-PROTOCOL.md` §8.5. Reviewed at PR time alongside the milestone's code diff and gap-analysis entry (Full tier) or alongside the milestone PR description (Standard/Lite).
