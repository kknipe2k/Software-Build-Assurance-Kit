...
When R5 passes and `docs/findings-to-illustrate.md` exists, Phase R is complete. The transition to **Phase A** (building the interactive app) is an **explicit, user-driven re-tier** — never automatic. The agent surfaces: *"given the findings you want to illustrate, here's what the app needs to do → that suggests `<tier>`; confirm or revise."* Phase A then runs at the chosen tier and **inherits Stage V**. (See `sbak/BUILD-PLAYBOOK.md` §3.9.)
