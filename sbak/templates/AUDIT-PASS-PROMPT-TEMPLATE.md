...
> The generic per-dimension audit pass, pasted into a **fresh** session at the start of a pass (`operating_mode: audit`, Phase P). Customize the `{{PLACEHOLDERS}}` per dimension — the eight concrete passes (P1–P8) instantiate this shape, seeding `<persona>` + `<checklist>` from the dimension's review baseline (shipped in M07.B). Validates against `sbak/STAGE-PROMPT-PROTOCOL.md` §8.7.
>
