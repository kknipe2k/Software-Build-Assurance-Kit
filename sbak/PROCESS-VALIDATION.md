...
**Tier mapping** (`OPERATING-MODES.md` §4.5): Lite — single-file fix, three phases in one fresh session each, brief retrospective. Full — multi-file or cross-cutting fix; same three phases; impact-analysis output may surface follow-ups (carried to `docs/tech-debt.md`); a waiver ADR is required if the fix changes a public surface.
...
**Tier mapping** (`OPERATING-MODES.md` §5.6; wired in `FRAMEWORK-CONFIG.md` §4.12): **Lite** — 1–2 dimensions, skip S2 triage + the challenge (a quick health check). **Full** — 3–4 dimensions by default, full setup, challenge the security-focused passes; all 8 dimensions + challenge-every-pass + a ranked remediation backlog by explicit choice.
