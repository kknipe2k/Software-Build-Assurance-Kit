...
| `two_axis` | Process + Product (Pattern axis omitted) | Full tier (the default) |
...
| **Orchestrator (Claude)** *(Full)* | Does NOT score retrospectives — that's the build session's job (it has the live context). The orchestrator *reads* finalized retrospectives to route Verifier findings, sequence stages, and aggregate the milestone summary at closeout. See `ORCHESTRATOR.md`. |
...
> **Tier note.** At **Lite** tier the orchestrator and build roles collapse into one session — "Build session (Claude)" and "Orchestrator" above are the same session. The retrospective is a brief paragraph (`retro_depth: brief`). The orchestrator row applies only at Full.
...
**Lane per gate (part of the gates above, not a new number).** Every gate names the `scripts/verify-local.cjs` lane it requires, and "tests passed" means the named lane, never a narrower one. **Per push (G1's commit/push discipline):** `fast` — the tests the diff affects by the runner's own selection, the open milestone's own tests, and the `always`-tagged guards; a widening to full is printed, never silent. **Stage-end (G4 for a stage):** `stage`, the same over the whole milestone diff. **Stage V (G6) and Stage R (G7):** `release` — the Verifier is the adversary and runs every test on every leg; a green `fast` lane is not Stage-V evidence and a findings file that cites one is Not Sound. **Closeout (G4 for a milestone, this evidence discipline):** `release`, then `--reconcile` proving the union pin (every discovered test in exactly one of settled / active; the last fast run plus the settled set covers every discovered test; release passed at this head) before the settled marker is written. **Version tags (G16's release end):** `release` in the tag workflow. The narrowing cannot silently widen or shrink: a test that falls out of every lane is a closeout failure naming the file, not a gap.
...
**`operating_mode: bug_fix`** — three gates, mapping to the three phases (the bug_fix workflow is `BUILD-PLAYBOOK.md` §3.7):
...
**Tier mapping:** Lite — single-file fix, three phases in one fresh session each, brief retrospective. Full — multi-file or cross-cutting fix; same three phases; impact-analysis output may surface follow-ups (carried to `docs/tech-debt.md`); a waiver ADR is required if the fix changes a public surface.

**`operating_mode: research_publish`** — five gates across the hybrid two-phase shape (the research_publish workflow is `BUILD-PLAYBOOK.md` §3.9; the research method is grounded STORM). The `R*` gates sit on **Phase R** (research; always Lite *process*); the `A*` gates sit on **Phase A** (the interactive app, at its re-tiered tier). All five live off the numbered line like every mode gate:
...
**Tier mapping** (wired in `FRAMEWORK-CONFIG.md` §4.12): **Lite** — 1–2 dimensions, skip S2 triage + the challenge (a quick health check). **Full** — 3–4 dimensions by default, full setup, challenge the security-focused passes; all 8 dimensions + challenge-every-pass + a ranked remediation backlog by explicit choice.
...
- **Closeout consumption: the stamp only counts once the human-drive answers are typed.** At a **closeout** retro, when the gate arms — tier Full AND the spec carries the three-part IRL/HITL plan (`sbak/templates/IRL-HITL-PLAN.md`) — the retrospective must also carry a typed `human-drive` fenced block (`drove:` / `verified:` / `recorded:`, the human's answers transcribed, each non-blank and non-placeholder) **before the friction stamp satisfies the close-gate**. Same pre-commit gate (`validators/validate-retrospective.cjs`); the arming detection is shared with the presence floor (`validators/validate-irl-plan.cjs`, via `validators/lib/irl-plan.cjs`), so the section the floor demands is exactly the one closeout consumes. The failure this consumes: a milestone can close green — valid stamp, clean gates — with no human ever running the app, because the plan section existed with no consumer. Not armed (Lite / incomplete section / no `project-config.md`) → a **visible n/a** NOTE, never a silent pass.
