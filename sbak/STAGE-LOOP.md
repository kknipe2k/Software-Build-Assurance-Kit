...
4. **Gate 2 — red.** Builder writes failing tests, surfaces them. Approve or Orch round-trip. **The release rides the packet (M22 ruling 5):** when the verdict is "release," the Orch packet ends with the explicit line `RED-RELEASE: approved — builder, run node scripts/approve-red.cjs`, and the Builder acts **only on that line** (the fence's ask prompt is your release click). Approval language without the line **is not a release** — the Builder stops and tells you `/approve-red` is required.
5. **Gate 3 — stage-end.** Builder implements to green, surfaces gates + retro + draft commit. Approve or Orch round-trip. **The stamp is asked-and-transcribed (M22 ruling 4):** at approval the builder asks for your verdict (pass/fail + optional note) and transcribes your reply verbatim into the retro's user-stamp block, marked `transcribed:` — you never hand-edit the retro; the verdict's content stays yours alone.
